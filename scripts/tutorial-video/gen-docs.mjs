 ...
 * `pnpm run tutorial:record`, see e2e/tutorials/tutorial.ts)
...
if (!slugs.length) {
  ...
  console.error("  PLAYWRIGHT_BASE_URL=<url> pnpm run tutorial:record");
  process.exit(1);
}
...
for (const slug of slugs) {
  ...
  const lines = [
    ...
    `> Regenerate: \`pnpm run tutorial:record\` then \`pnpm run tutorial:docs ${slug}\`.`,
    "",
  ];
