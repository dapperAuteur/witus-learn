...
- Tex. Const. art. IV (Executive Department). https://statutes.capitol.texas.gov/Docs/CN/htm/CN.4.htm`,
    ...
    // ── Section 2: The three state branches ──────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "In what year did Texas adopt its current constitution, and why is it amended so often?",
          answer: "1876. It writes many specifics directly into the constitution, so changing state policy often requires a constitutional amendment (700+ proposed, 500+ approved).",
        },
        {
          prompt: "What does 'plural executive' mean in the Texas Constitution?",
          answer: "Executive power is scattered among several separately-elected statewide officers rather than concentrated in the governor.",
        },
      ],
...
- Texas Legislative Council. (n.d.-b). *Texas legislative glossary.* TLC.texas.gov. https://tlc.texas.gov/glossary`,
    },
    {
      ...
      recallContent: [
        {
          prompt: "What is the size and term length of each chamber of the Texas Legislature?",
          answer: "The House has 150 members with 2-year terms; the Senate has 31 members with 4-year terms.",
        },
        {
          prompt: "Why is the Texas Lieutenant Governor often called the most powerful office in the state?",
          answer: "As President of the Senate, the Lt. Governor presides over the Senate, appoints its committee members and chairs, and assigns bills to committee.",
        },
      ],
...
- Texas Court of Criminal Appeals. (n.d.). *About the Court.* Txcourts.gov. https://www.txcourts.gov/cca/`,
    ...
    // ── Section 3: How a bill becomes a state law ────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "What are Texas's two highest courts, and which cases does each end?",
          answer: "The Supreme Court of Texas is the last word on civil cases; the Court of Criminal Appeals is the last word on criminal cases (a bifurcated top since 1891).",
        },
        {
          prompt: "How does Texas select the judges on its two highest courts?",
          answer: "In partisan statewide elections (party label on the ballot) to six-year terms.",
        },
      ],
...
- Texas House of Representatives. (n.d.). *About the Texas House.* House.texas.gov. https://house.texas.gov/about`,
    ...
    // ── Section 4: Local government in Texas ─────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "When does the Texas Legislature hold its regular session, and for how long?",
          answer: "Only in odd-numbered years, convening the second Tuesday in January for a maximum of 140 days; only the governor can call special sessions.",
        },
        {
          prompt: "How can the Texas Legislature override a governor's veto, and why is that often not possible?",
          answer: "By a two-thirds vote of each chamber, but because the session is short (140 days) many vetoes arrive after the Legislature has adjourned, so they are never subject to an override vote.",
        },
      ],
...
- Tex. Const. art. XI (Municipal Corporations, home-rule cities). https://statutes.capitol.texas.gov/Docs/CN/htm/CN.11.htm`,
    ...
    // ── Section 5: Elections & ballot access ─────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "How many counties does Texas have, and who governs each one?",
          answer: "254 counties, the most of any state, each governed by a commissioners court made up of a county judge plus four commissioners elected from precincts.",
        },
        {
          prompt: "What common local government layer does Texas NOT have?",
          answer: "Townships. Texas has no township layer between the county and the city; unincorporated areas are largely served by the county plus special districts.",
        },
      ],
      body: `Elections are run by each state, so Texas's rules are specifically Texas's, and several of them are notably **strict**. (Because rules like these change, the authoritative and current source is always the Texas Secretary of State / VoteTexas at [votetexas.gov](https://votetexas.gov) and [sos.state.tx.us](https://sos.state.tx.us).)

**Strict photo voter ID, with a fallback declaration.** To vote in person in Texas, you generally must present one of a short list of accepted **photo IDs**: for example a Texas driver license, a Texas Election Identification Certificate, a Texas personal ID card, a Texas handgun license, a U.S. military ID with photo, a U.S. citizenship certificate with photo, or a U.S. passport (VoteTexas.gov, n.d.-a). For voters aged **18 to 69**, most of these may be expired **no more than four years**; for voters **70 or older**, an otherwise-valid ID may be expired for any length of time. A voter who cannot reasonably obtain one of the accepted photo IDs may sign a **Reasonable Impediment Declaration** and show a supporting document (like a utility bill, bank statement, government check, or voter registration certificate).
...
- Tex. Const. art. XVII (Mode of Amending the Constitution). https://statutes.capitol.texas.gov/Docs/CN/htm/CN.17.htm`,
    ...
    // ── Section 6: Get involved where you live ───────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "What ID do you need to vote in person in Texas, and what is the fallback?",
          answer: "An accepted photo ID; a voter who cannot reasonably obtain one may sign a Reasonable Impediment Declaration and show a supporting document.",
        },
        {
          prompt: "Can a Texas citizen put a new state law on the statewide ballot by petition?",
          answer: "No. Texas has no statewide citizen initiative; only the Legislature can propose amendments (by a two-thirds vote), which voters then ratify.",
        },
      ],
...
**Register and vote: mind Texas's deadlines.** Register (and check your registration) **at least ~30 days before** any election you want to vote in, since Texas has no same-day registration. Bring an **accepted photo ID** to the polls, or, if you can't get one, be ready to sign a **Reasonable Impediment Declaration** with a supporting document. Start at [VoteTexas.gov](https://votetexas.gov) (VoteTexas.gov, n.d.-b).

**Contact your state legislators, but remember the biennial clock.** You have **one state representative** and **one state senator** in the Legislature, on top of your members of Congress. Because Texas has **no citizen initiative**, lobbying your legislators is *the* way to change state law here. And because the Legislature only meets every other year for 140 days, **timing matters**: engaging *before and during* a regular (odd-year) session is far more effective than after it adjourns. Look up your legislators and track bills at **[capitol.texas.gov](https://capitol.texas.gov)** (Texas Legislature Online), where you can read bills and find committee schedules.

**Testify or submit comment on a bill.** Texas's committee hearings are where most bills live or die, and they're **open to public testimony**. Watch a committee's calendar on [capitol.texas.gov](https://capitol.texas.gov), and sign up to testify or submit written comment on a bill you care about: comment carries the most weight *before* the committee vote.
...
:::reveal Name three specific ways to get involved in Texas government, and explain why the *timing* of contacting your state legislators matters so much in Texas given its biennial session and lack of a citizen initiative. ||| Three ways: contact your legislators and comment before committee votes, show up at your commissioners court, city council, and school or special-district boards, and vote your partisan judicial races thoughtfully. Timing matters enormously because the Texas Legislature meets only once every two years (biennial) and there's no citizen initiative, so if you miss that narrow window before a committee vote, the next chance to shape a state law may be two years away.
