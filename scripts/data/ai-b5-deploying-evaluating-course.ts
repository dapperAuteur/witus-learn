...
The throughline is NIST's insight that trustworthy AI is **continuous**: you *Measure* (steps 2–3, 6) and *Manage* (steps 4–5, 7) on an ongoing basis, because the model, your data, and your users all change (NIST, 2023).

Each remaining section of this course is one arc of this loop: **Evaluation** (steps 1–3, 6), **Guardrails & Observability** (step 4), **Reliability & Cost** (step 4, deeper), and **Operate Responsibly** (steps 5, 7).
...
- **Canary release.** Send the new version to a *small slice* of traffic first (say 1–5%), watch quality, error rate, latency, and cost, and only widen if it holds. If it misbehaves, you've exposed a few users, not all of them.
...
- **Single-answer grading** — score one output against a rubric ("rate faithfulness 1–5").
    ...
    },
    {
      ...
      exercise: {
        ...
        items: [
          ...
          {
            prompt:
              "Releasing a new version to a small slice of traffic (say 1–5%) first, then widening only if it holds, is a ___ release.",
            answer: "canary",
...
**Stage the rollout.** Don't flip a new AI feature on for everyone at once. Walk it out: **internal → small canary (1–5%) → gradual widen → full**, checking quality, safety, latency, and cost dashboards at each gate, with your kill switch armed. Each step bounds the blast radius of a surprise (Sections 2 and 5).
