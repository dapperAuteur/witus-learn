...
Code.org. (n.d.). *How AI works* (curriculum & videos for grades 6-12). https://code.org/ai`,
    },
    {
      ...
      recallContent: [
        {
          prompt:
            "Which step of the robot's sense, think, act loop does adding AI mainly upgrade?",
          answer:
            "The think step. Instead of following only rules a person typed in ahead of time, the robot runs a model that learned a pattern from examples. Sense and act stay the same.",
        },
        {
          prompt:
            "Does adding AI replace a robot's sensors and actuators?",
          answer:
            "No. AI is a smarter brain for the middle think step, not a new body. The robot still needs the same sensors and actuators, like a delivery robot's wheels and camera.",
        },
      ],
      ...
      recallContent: [
        {
          ...
          answer:
            "Programmed rules are if/else instructions a person writes in advance: predictable and good for clear, countable cases. Learned behavior finds the pattern from many labeled examples: good for messy situations, but a guess that can be wrong.",
        },
        {
          prompt:
            "How do real robots often combine both approaches for a safety action like stopping at a stop sign?",
          answer:
            "They use learned AI to recognize the stop sign in the camera image, then a simple, hard rule (if a stop sign is detected, brake) to act. The safety-critical action stays a plain, readable rule because a learned guess can be wrong.",
        },
      ],
...
MIT RAISE. (n.d.). *Day of AI: free K-12 AI literacy curriculum*. https://raise.mit.edu/`,
    },
    {
      ...
      recallContent: [
        {
          prompt:
            "A camera only records pixels. What is computer vision, and how does it learn?",
          answer:
            "Computer vision teaches a computer to make sense of images and video, like turning a grid of pixels into 'that's a stop sign.' It learns by seeing thousands of labeled pictures, finding the pattern, then guessing on a new scene.",
        },
        {
          prompt:
            "What are the two honest truths about computer vision that matter for safety?",
          answer:
            "It is a guess, not a fact, so it can be confidently wrong (a shadow or snow can fool it). And it is only as good as its examples, so situations left out of training cause bias, and a robot might act on the mistake.",
        },
      ],
...
Arduino. (n.d.). *About Arduino* (machine learning for microcontrollers with TensorFlow Lite). https://www.arduino.cc/en/about`,
    ...
    // ── Part 3: Robots that move on their own ──────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt:
            "What is the difference between sound recognition and voice (speech) recognition?",
          answer:
            "Sound recognition tells what a sound is, like a clap, a doorbell, or a smoke alarm. Voice or speech recognition turns spoken words into text or commands. Both learn from many labeled example sounds, then guess on a new one.",
        },
        {
          prompt:
            "Why might a voice-controlled robot work for the person who trained it but struggle with a friend?",
          answer:
            "If most training examples were one accent in a quiet room, the model may not match a different voice or a noisy kitchen. It hears patterns, not meaning, so missing examples cause bias, the same problem as in vision.",
        },
      ],
...
NASA JPL. (n.d.). *Mars 2020 Perseverance rover components*. NASA Science. https://science.nasa.gov/mission/mars-2020-perseverance/rover-components/`,
    ...
    // ── Part 4: Teaching a robot ───────────────────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt:
            "Why must a Mars rover drive itself, when a sidewalk delivery robot could be watched by a human?",
          answer:
            "Mars is so far away that a person cannot steer in real time, so the rover makes its own routine driving decisions rather than waiting minutes for every command from Earth. Perseverance's cameras help ensure a safe path.",
        },
        {
          prompt:
            "What is the trade-off as a robot becomes more independent?",
          answer:
            "More independence is useful, since a rover cannot wait for every command, but also more demanding: the more a robot decides on its own, the more carefully it must be built and tested, because no human is there to catch a mistake in the moment.",
        },
      ],
...
> **Drill below.** Lock in Parts 1 to 4 before we get to safety and ethics.
...
Arduino. (n.d.). *About Arduino* (ML on microcontrollers, TensorFlow Lite). https://www.arduino.cc/en/about`,
      exercise: {
        instructions:
          "Fill in the missing word from Parts 1 to 4. Spelling is forgiving: the explanation shows the idea you should have.",
        items: [
          {
            prompt:
    ...
    // ── Part 5: Doing it safely & responsibly ──────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt:
            "What are the three stages of teaching a robot a new skill by example, as in micro:bit CreateAI?",
          answer:
            "Collect data (record labeled examples while a sensor watches), train and test the model (it finds the pattern that separates examples like a wave from a clap, and you see where it is confused), then improve and use it (add better examples, then run it live on new movements).",
        },
        {
          prompt:
            "When a trained model keeps confusing two movements, how does an engineer fix it?",
          answer:
            "You give it clearer, more varied examples (better data), not more rules. Better examples beat more rules, and one-sided data, like only slow gentle waves, creates bias. The data is how you teach.",
        },
      ],
...
NASA. (n.d.). *Robotics* (autonomous systems operating independently). https://www.nasa.gov/robotics/`,
    },
    {
      ...
      recallContent: [
        {
          prompt:
            "Why must engineers design, on purpose, when a robot may act alone versus stop and ask a human?",
          answer:
            "Because an AI-powered robot's decision is a guess that can be confidently wrong, and the world throws situations its examples missed. When a robot moves, a wrong guess can bump, drop, or collide with something real, so risky actions need a human in the loop.",
        },
        {
          prompt:
            "For the highest-stakes action, why use a plain hard rule instead of leaving it to the learned AI?",
          answer:
            "A plain, readable rule (like: if a person is detected within 1 meter, stop) is predictable, while a learned guess can be wrong. The learned AI does the clever recognizing; a simple, checkable rule does the protecting.",
        },
      ],
...
Code.org. (n.d.). *How AI works* (ethics alongside technical understanding). https://code.org/ai`,
    ...
    // ── Part 6: Build & think project ──────────────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt:
            "What does automation do to jobs, and what are the fair questions to ask?",
          answer:
            "Automation (machines doing work on their own) can remove some jobs, often the dull, dirty, or dangerous ones, while creating new ones like designing, training, testing, and repairing the robots. The fair questions: how do we help people whose jobs change, and keep new opportunities open to everyone?",
        },
        {
          prompt:
            "When a machine acts on its own and something goes wrong, who is responsible?",
          answer:
            "A person is still accountable for how the machine was built, trained, and allowed to act. 'The AI did it' is never an excuse, because humans chose the data, set the rules, and decided what it could do alone.",
        },
      ],
...
Arduino. (n.d.). *About Arduino* (on-device ML with TensorFlow Lite). https://www.arduino.cc/en/about`,
    },
    {
      ...
      exercise: {
        instructions:
          "Fill in the missing word from Parts 5 to 6. Spelling is forgiving: get the idea right.",
        items: [
          {
            prompt:
