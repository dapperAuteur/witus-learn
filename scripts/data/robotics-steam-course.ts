...
NASA. (n.d.-b). *Mars 2020 Perseverance rover components*. NASA Science. https://science.nasa.gov/mission/mars-2020-perseverance/rover-components/`,
    },
    {
      ...
      recallContent: [
        {
          prompt: "What three-step loop does every robot follow?",
          answer: "Sense, think, act. A robot senses the world, thinks about what it senses, then acts to do something useful.",
        },
        {
          prompt: "What do engineers call the parts that sense, think, and act?",
          answer: "A sensor senses (it measures something), a controller thinks (the brain that decides), and an actuator acts (a motor, light, or wheel that makes something happen).",
        },
      ],
...
NASA. (n.d.-a). *Robotics*. National Aeronautics and Space Administration. https://www.nasa.gov/robotics/`,
    ...
    // ── Section 2: Electricity & circuits ──────────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "What are the three D's of jobs that robots are especially good for?",
          answer: "Dull (very repetitive), dirty (messy or unhealthy), and dangerous (unsafe for people).",
        },
        {
          prompt: "Are robots smarter than people, and who does the imagining and deciding?",
          answer: "No. Robots are fast, tireless, and precise, but they only do what they are built and programmed to do. Humans still imagine the problem, design the robot, and decide what is worth doing.",
        },
      ],
...
Micro:bit Educational Foundation. (n.d.). *micro:bit features overview*. https://microbit.org/get-started/features/overview/`,
    ...
    // ── Section 3: Input → Process → Output ────────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "What three parts does a basic circuit need?",
          answer: "A power source (a battery that pushes the electricity), a path (wires for the current to flow through), and a load (something useful like a bulb, buzzer, or motor).",
        },
        {
          prompt: "Why must a circuit loop be closed, and what is a switch?",
          answer: "Electricity can only flow around a complete, closed loop, so any gap stops it. A switch is a controlled gap: closing it turns the load on, opening it turns the load off.",
        },
      ],
...
Micro:bit Educational Foundation. (n.d.). *micro:bit features overview*. https://microbit.org/get-started/features/overview/`,
    ...
    // ── Section 4: Coding logic & algorithms ───────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "What three words do computer scientists use for sense, think, act?",
          answer: "Input, process, output. Input is information coming in, process is the program deciding what to do, and output is the result going out.",
        },
        {
          prompt: "Give an example of an input, a process, and an output on a robot.",
          answer: "Input comes from a sensor or button, process is the program that decides what the input means, and output is an action like a light, a motor, or a sound.",
        },
      ],
      ...
      recallContent: [
        {
          ...
          answer: "A step-by-step set of instructions to get something done, like a recipe. A computer follows it exactly and in order.",
        },
        {
          prompt: "What is sequencing, and why does the order of steps matter to a computer?",
          answer: "Sequencing means the order of the steps matters. A computer is extremely literal and does exactly what you say in exactly the order you say it, so a missing or out-of-order step breaks the result.",
        },
      ],
      ...
      recallContent: [
        {
          ...
          answer: "A loop repeats steps over and over instead of writing them out many times, such as repeat 50 times: step forward.",
        },
        {
          prompt: "What does a conditional (if/else) let a robot do?",
          answer: "It runs code only when a condition is true, and can do something else otherwise, so a robot can react differently to different inputs. That is how it seems to decide.",
        },
      ],
...
IEEE TryEngineering. (n.d.). *Engineering STEM lesson plans & activities*. https://tryengineering.org/explore-resources/lesson-plans/`,
    ...
    // ── Section 6: Platforms ───────────────────────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "What does it mean to iterate in the engineering design process?",
          answer: "To iterate is to go back, improve your design, and try again. Engineers loop through building, testing, and evaluating until a solution works.",
        },
        {
          prompt: "Why do engineers treat a failed test as useful, and what is a prototype?",
          answer: "A failed test is a normal step that teaches you what to change next, not the end. A prototype is a quick, rough first test version, not the finished product.",
        },
      ],
...
Arduino. (n.d.-b). *Getting started with Arduino products*. https://www.arduino.cc/en/Guide/`,
    ...
    // ── Section 7: Real-world robots ───────────────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "How are the micro:bit and Arduino different as beginner platforms?",
          answer: "The micro:bit has many sensors and outputs built in, so you can program it the first day without wiring. Arduino is a controller you wire your own sensors and motors to, so it teaches more about circuits.",
        },
        {
          prompt: "What does it mean that Arduino is open-source?",
          answer: "Its designs are shared freely, so anyone can study, use, and build on them.",
        },
      ],
...
NASA. (n.d.-b). *Mars 2020 Perseverance rover components*. NASA Science. https://science.nasa.gov/mission/mars-2020-perseverance/rover-components/`,
    ...
    // ── Section 8: Ethics & the future ─────────────────────────────────────────
    {
      ...
      recallContent: [
        {
          prompt: "Name three fields where real robots use the sense, think, act loop.",
          answer: "Factories (robotic arms welding, painting, and lifting), medicine (robotically-assisted surgical systems), and space (NASA rovers like Perseverance on Mars).",
        },
        {
          prompt: "According to the FDA, why is a robotically-assisted surgical system not fully a robot on its own?",
          answer: "Because it cannot perform surgery without direct human control. The surgeon stays in charge of every move, so the robot and the person work together.",
        },
      ],
