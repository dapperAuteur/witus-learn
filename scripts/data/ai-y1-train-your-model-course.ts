...
- Common Sense Education. *AI literacy lessons* (how AI is trained on data). https://www.commonsense.org/education/collections/ai-literacy-lessons-for-grades-6-12`,
      recallContent: [
        {
          prompt: "How do machines actually learn, if not by magic or hand-typed rules?",
          answer: "They learn from lots of examples: you show the computer many examples and it finds the pattern itself. That is machine learning (ML).",
        },
        {
          prompt: "What is the trained pattern-finder that machine learning builds called?",
          answer: "A model. It is the thing you train, and it makes an educated guess based on patterns it saw, so it can be confident and still wrong.",
        },
      ],
    },
    {
...
- Code.org. *How AI Works* (training data and how models learn). https://code.org/curriculum/how-ai-works`,
      recallContent: [
        {
          prompt: "What is the difference between a feature and a label?",
          answer: "A feature is a clue the model looks at (the 'what it notices'), like color or shape. A label is the right-answer category you are teaching (the 'what it should say'), like apple or banana.",
        },
        {
          prompt: "What is supervised learning?",
          answer: "Training with examples that come WITH labels, meaning a human supplied the right answers. Unsupervised learning gives no labels and asks the model to group similar examples on its own.",
        },
      ],
    },
    {
    ...
    // ── Part 2: Build It! ──────────────────────────────────────────────────────
    {
      ...
      body: `Time to actually do it. You're going to train a real image model (no coding, no app to install, and **no account needed**) using a free tool from Google called **Teachable Machine**. It runs right in your web browser at **[teachablemachine.withgoogle.com](https://teachablemachine.withgoogle.com)**.
...
1. **Open the tool.** Go to [teachablemachine.withgoogle.com](https://teachablemachine.withgoogle.com) and choose **Get Started → Image Project → Standard image model.**
...
- Google. *Teachable Machine 2.0 makes AI easier for everyone* (overview & how it works). https://blog.google/innovation-and-ai/products/teachable-machine/`,
      recallContent: [
        {
          prompt: "What does 'garbage in, garbage out' (GIGO) mean in machine learning?",
          answer: "A model is only as good as the examples you train it on. Messy, lopsided, or wrongly labeled examples make a messy, lopsided, or wrong model, no matter how fancy the computer is.",
        },
        {
          prompt: "Name two things that make training data good.",
          answer: "Any two of: enough examples per label, balanced counts across labels, correct labels, variety (different angles, lighting, backgrounds), and examples that look like what you will actually use it on.",
        },
      ],
    },
    {
...
- Code.org. *How AI Works* (how models are evaluated and improved). https://code.org/curriculum/how-ai-works`,
      recallContent: [
        {
          prompt: "What tool did you use to train an image model, and what did you need to get started?",
          answer: "Google's Teachable Machine, right in your web browser. It is free, needs no coding and no account, and your data stays on your own device.",
        },
        {
          prompt: "What are the basic steps to train an image model in Teachable Machine?",
          answer: "Start an Image project, name your two labels, collect varied and balanced training data (for example from your webcam), click Train Model, then try it live in the Preview panel.",
        },
      ],
    },
    {
...
- Code.org. *How AI Works* (machine learning concepts). https://code.org/curriculum/how-ai-works`,
      recallContent: [
        {
          prompt: "Why must you test a model on examples it has never seen before?",
          answer: "If you only test on the exact photos you trained it with, it looks perfect because it basically memorized them. New examples show how it really does. Accuracy is how often it gets the right answer, so right 9 of 10 times is 90%.",
        },
        {
          prompt: "What is confidence, and does high confidence mean the answer is correct?",
          answer: "Confidence is how sure the model is about a guess, shown as a percent. High confidence is NOT the same as being right: a model can be 99% confident and still wrong.",
        },
      ],
    },
...
- Common Sense Education. *AI literacy lessons* (AI bias and its real-world impacts). https://www.commonsense.org/education/collections/ai-literacy-lessons-for-grades-6-12`,
      recallContent: [
        {
          prompt: "What is overfitting, and what is the tell-tale sign of it?",
          answer: "Overfitting is when a model memorizes its training examples instead of learning the general pattern. The tell is that it is great on the training data but bad on new, real examples.",
        },
        {
          prompt: "Name one way makers fight overfitting.",
          answer: "Any of: add more examples, add more variety (different angles, lighting, backgrounds), test on fresh examples to catch it, and do not over-train on tiny data. A good model generalizes to things it never saw.",
        },
      ],
    },
    {
...
- Code.org. *How AI Works* (real-world AI: computer vision, chatbots, and ethics). https://code.org/curriculum/how-ai-works`,
      recallContent: [
        {
          prompt: "What is bias in a machine learning model, and where does it come from?",
          answer: "Bias is a model's unfair leaning. It comes from the examples: if your training data leaves people out or reflects unfair ideas, the model copies those mistakes. It usually is not on purpose.",
        },
        {
          prompt: "How can you make a fairer model?",
          answer: "Include the variety of people and things it will meet, balance your labels and groups (lopsided data makes a lopsided model), and ask 'who might this fail for?' before you ship it.",
        },
      ],
    },
    {
...
- Common Sense Education. *AI literacy lessons* (be a responsible, ethical user of AI). https://www.commonsense.org/education/collections/ai-literacy-lessons-for-grades-6-12`,
      recallContent: [
        {
          prompt: "Give two real-life places the same 'features in, label out' model shows up.",
          answer: "Any two of: a phone unlocking by your face, a photos app finding 'all dogs,' an email spam filter, a voice assistant hearing its wake word, video captions or song ID, and a bank flagging a suspicious charge.",
        },
        {
          prompt: "Because these everyday models are trained on examples, what two things can go wrong with them?",
          answer: "They can be wrong (they are only guessing), and they can pick up bias from their training data, working worse for people the examples left out. That matters most for high-stakes uses like loans, jobs, or policing.",
        },
      ],
    },
    {
