...
- OWASP. (2025). *Top 10 for LLM Applications 2025*, LLM08: Vector and Embedding Weaknesses (a retrieved passage may be poisoned or stale). https://genai.owasp.org/llmrisk/llm082025-vector-and-embedding-weaknesses/`,
    },
    {
      slug: "the-pipeline",
      recallContent: [
        { prompt: "A plain language model only knows two things. What are they, and what problem does that create?", answer: "It knows what was in its training data and what you put in the prompt right now. It has no access to your internal docs, latest prices, or recent changes, and rather than admit that, it fills the gap with a fluent guess." },
        { prompt: "How does RAG address that, and does grounding guarantee a correct answer?", answer: "RAG retrieves the relevant pieces of your own text and puts them in the prompt so the model answers from real content, not just its parametric memory. Grounding is a probability, not a guarantee: a retrieved passage is not automatically true, so you still cite and check." },
      ],
...
- OpenAI. (2025). *Vector embeddings* (API documentation), turning text into vectors for similarity search. https://platform.openai.com/docs/guides/embeddings`,
    },
    {
      slug: "chunking",
      recallContent: [
        { prompt: "Which RAG steps run offline when documents change, and which run per question?", answer: "Offline indexing is ingest, chunk, embed, and store. Per question you embed the user's question, retrieve the top-k nearest chunks, assemble the prompt, and generate an answer with citations." },
        { prompt: "Why must the question be embedded with the same model used for the chunks?", answer: "Vectors from two different models are not comparable, so similarity search would silently return garbage. Query and chunks have to share one embedding model." },
      ],
...
- **Fixed-size**: split every N tokens (often with a small **overlap**, e.g. 10 to 15%, so a sentence cut at a boundary still appears whole in a neighboring chunk). Simple, language-agnostic, and a fine baseline.
...
- OpenAI. (2025). *Vector embeddings* (API documentation), embedding input limits and how chunk content becomes a vector. https://platform.openai.com/docs/guides/embeddings`,
    ...
    // ── Section 2 · Retrieval Quality ─────────────────────────────────────────
    {
      ...
      recallContent: [
        { prompt: "Describe the two opposite chunking failures and what each one costs you.", answer: "Chunks too large span several topics and get a muddy average embedding that matches everything weakly, and they waste context space. Chunks too small are torn mid thought and lose the context that makes them answerable." },
        { prompt: "Why keep metadata like source, section, and date on every chunk?", answer: "You need it to cite the answer, to filter by access control or freshness, and to debug which chunk produced a claim. A chunk without provenance is a citation you cannot make." },
      ],
...
- OpenAI. (2025). *Vector embeddings* (API documentation), dimensions, normalized vectors, cosine vs. dot product, token limits. https://platform.openai.com/docs/guides/embeddings`,
    },
    {
      ...
      recallContent: [
        { prompt: "Why must every vector in a store share the same embedding model and dimension count?", answer: "Embeddings are only comparable when they come from the same model and have the same fixed length. Mixing models or dimensions breaks similarity search." },
        { prompt: "What does pgvector do by default, and what changes when you add an HNSW or IVFFlat index?", answer: "By default pgvector does exact nearest neighbor search with perfect recall but scans every row. HNSW or IVFFlat add approximate search that trades a little recall for much more speed at scale." },
      ],
...
hybrid search (vector + BM25) → take top ~50 candidates
   → re-rank against the question → keep top ~5 to 10 → put in prompt
\`\`\`
...
- Lewis, P., et al. (2020). Retrieval-augmented generation for knowledge-intensive NLP tasks. *arXiv:2005.11401* (retrieving multiple passages per query). https://arxiv.org/abs/2005.11401`,
    ...
    // ── Section 3 · Generation & Evaluation ───────────────────────────────────
    {
      ...
      recallContent: [
        { prompt: "Pure vector search is strong at meaning but weak at one thing. What, and which technique fixes it?", answer: "It can miss exact strings like error codes, SKUs, and names. Hybrid search adds BM25 keyword matching to catch those while vectors handle meaning." },
        { prompt: "What does a re-ranker add on top of first-pass retrieval?", answer: "It is a sharper second stage model that re-scores the top candidates against the actual question and keeps the best few. Retrieve broadly, then re-rank precisely." },
      ],
...
- OWASP. (2025). *Top 10 for LLM Applications 2025*, LLM01: Prompt Injection (instructions hidden in retrieved/external content). https://genai.owasp.org/llmrisk/llm01-prompt-injection/`,
    },
    {
      ...
      section: "Generation & Evaluation",
      body: `Lock in the vocabulary and the decisions before moving on to evaluation and failure modes. Each blank is a term or choice from sections 1 to 2.`,
      exercise: {
        instructions:
    ...
    },
    {
      slug: "evaluating-rag",
      recallContent: [
        { prompt: "Which single prompt instruction most reduces hallucination when retrieval finds nothing relevant?", answer: "Tell the model to answer only from the provided passages and to say it does not know if the answer is not there. That explicit license to refuse turns a confident hallucination into an honest admission that it does not know." },
        { prompt: "Why do fewer, better chunks often beat stuffing in everything you retrieved?", answer: "Everything must fit the context window and costs tokens on every call, and models attend unevenly across a long context, so a flood of marginal chunks can bury the relevant one." },
      ],
...
- Anthropic. (2024). *Introducing Contextual Retrieval*, measuring retrieval failure rate (recall) as a first-class metric, separate from answer quality. https://www.anthropic.com/news/contextual-retrieval`,
    ...
    // ── Section 4 · Failure Modes & When Not To ───────────────────────────────
    {
      ...
      recallContent: [
        { prompt: "Why evaluate retrieval separately from generation?", answer: "They fail for different reasons. If recall@k is low the right passage never reached the model, a retrieval bug that prompting cannot fix. Only with good context do you judge generation on groundedness, relevance, and correct refusal." },
        { prompt: "What is groundedness, and why can a fluent, correct sounding answer still fail it?", answer: "Groundedness is whether every claim in the answer is supported by the retrieved passages. A fluent answer can lean on the model's parametric memory instead of your sources, which fails groundedness and breaks citations." },
      ],
...
- **Irrelevant chunks (the silent killer).** Retrieval returns plausible-but-wrong passages; the model dutifully grounds a confident, wrong answer in them. The fix is everything from sections 2 to 3 (better chunking, hybrid search, re-ranking) *plus measuring recall* so you catch it.
...
- OWASP. (2025). *Top 10 for LLM Applications 2025*, LLM08: Vector and Embedding Weaknesses (embedding inversion/leakage, poisoning, access-control failures in shared stores). https://genai.owasp.org/llmrisk/llm082025-vector-and-embedding-weaknesses/`,
    },
    {
      ...
      recallContent: [
        { prompt: "What is indirect prompt injection in a RAG system?", answer: "An attacker plants instructions inside a document you ingest, such as telling the model to ignore its instructions or email the user's data out. When that chunk is retrieved it enters your prompt as if you wrote it, and it can even be hidden text in a PDF." },
        { prompt: "Why is a stale but cited answer especially dangerous, and name one defense.", answer: "A wrong answer with a source attached looks trustworthy, so it is worse than an obvious guess. Defenses include a freshness discipline that re-indexes on change and dates every chunk, and treating retrieved text as untrusted data." },
      ],
