...
- OpenAI. (2025). *Model optimization (fine-tuning) guide*: using shorter prompts and proprietary data via fine-tuning. https://platform.openai.com/docs/guides/fine-tuning`,
      recallContent: [
        {
          prompt: "What are the two big stages that build a modern chat model?",
          answer: "Pretraining, where the model learns to predict the next token on a huge pile of text and becomes a fluent but unhelpful base model, and post-training, where instruction tuning plus RLHF shape it into a helpful assistant.",
        },
        {
          prompt: "How does your own fine-tuning relate to how a model was originally built?",
          answer: "Fine-tuning is a tiny, targeted version of stage 2 (post-training) run on your own data. It nudges an already capable model toward your task, tone, or format rather than teaching it to think.",
        },
      ],
    },
...
- National Institute of Standards and Technology. (2023). *AI Risk Management Framework (AI RMF 1.0)*: see the "Map" function (frame the problem before choosing a solution). https://www.nist.gov/itl/ai-risk-management-framework`,
      recallContent: [
        {
          prompt: "What does fine-tuning actually change, and using what kind of data?",
          answer: "It continues training an already trained model on a smaller, focused dataset of (input, ideal output) pairs, updating the model's own weights so it produces outputs more like yours. You end with a customized version of the model.",
        },
        {
          prompt: "Why is fine-tuning usually the wrong tool for answering questions about your own documents?",
          answer: "Fine-tuning teaches behavior and form, not a reliable, updatable knowledge base. For facts that change and need to be citable, use retrieval (RAG) instead.",
        },
      ],
    },
    {
...
- Hugging Face. (2024). *PEFT (Parameter-Efficient Fine-Tuning) documentation.* https://huggingface.co/docs/peft/index`,
      recallContent: [
        {
          prompt: "What is the honest order of operations before you reach for fine-tuning?",
          answer: "Prompt engineering first because it is cheap, instant, and reversible, then RAG when the gap is missing or changing knowledge, and only then fine-tuning when the gap is a behavior, form, style, or narrow-skill problem the first two can't reach.",
        },
        {
          prompt: "The model's answers are correct but never in your required JSON shape. Which tool, and why not RAG?",
          answer: "Fine-tuning. The gap is form and format, not missing facts, so retrieval would not help. RAG is for knowledge gaps, when the model doesn't know something.",
        },
      ],
    },
...
**Start small, then grow with evidence.** Anthropic's Bedrock guidance is to begin with as few as **50 to 100 rows of high-quality data**, fine-tune, and *evaluate*, then add data only where the evaluation shows weakness (Anthropic, 2024). OpenAI similarly frames fine-tuning data as a curated set of correct example responses (OpenAI, 2025). Don't bulk-collect first; curate, test, and target the gaps.
...
- OpenAI. (2025). *Model optimization (fine-tuning) guide*: supervised examples of correct responses; dataset preparation. https://platform.openai.com/docs/guides/fine-tuning`,
      recallContent: [
        {
          prompt: "What is the difference between supervised (instruction) tuning and preference tuning?",
          answer: "Supervised fine-tuning (SFT) trains on (prompt, ideal response) pairs and is the workhorse for most API fine-tuning. Preference tuning trains on (prompt, better response, worse response) triples so the model learns which style or judgment you prefer, for example OpenAI's DPO.",
        },
        {
          prompt: "What is LoRA, and why does it let a solo builder fine-tune a large open model on one GPU?",
          answer: "LoRA is a parameter-efficient (PEFT) method that freezes the pretrained weights and injects small trainable low-rank matrices. It cuts trainable parameters by orders of magnitude and adds no extra inference latency once merged, so the training fits on a single GPU.",
        },
      ],
    },
    {
...
- National Institute of Standards and Technology. (2023). *AI Risk Management Framework (AI RMF 1.0)*: see the "Measure" function (measure performance on held-out, representative data). https://www.nist.gov/itl/ai-risk-management-framework`,
      recallContent: [
        {
          prompt: "For a fine-tuning dataset, what matters most, and how many rows should you start with?",
          answer: "Quality beats quantity. Start with as few as 50 to 100 high-quality, consistent, representative (input, ideal output) pairs, fine-tune, evaluate, then add data only where the evaluation shows weakness.",
        },
        {
          prompt: "What must you always reserve from your data before training, and why?",
          answer: "A held-out validation or test set the model never trains on, so you can measure real performance (whether it generalizes) instead of memorization.",
        },
      ],
    },
    {
...
- National Institute of Standards and Technology. (2023). *AI Risk Management Framework (AI RMF 1.0)*: "Measure" function. https://www.nist.gov/itl/ai-risk-management-framework`,
      recallContent: [
        {
          prompt: "What is an epoch, and why is falling training loss not the real goal?",
          answer: "An epoch is one full pass over your training data. Falling training loss only shows the model fitting data it has already seen; the goal is generalizing to new inputs, which you judge on held-out validation data.",
        },
        {
          prompt: "Training loss keeps dropping but validation performance stalls or declines. What is it, and what do you do?",
          answer: "That gap is overfitting: the model is memorizing examples instead of learning the pattern. Train for fewer epochs, use more varied data, and stop when validation stops improving.",
        },
      ],
    },
    {
...
- Anthropic. (2024). *Fine-tune Claude 3 Haiku in Amazon Bedrock*: managed fine-tuning; data stays in the customer's AWS environment. https://www.anthropic.com/news/fine-tune-claude-3-haiku`,
      recallContent: [
        {
          prompt: "Your fine-tune scores 88% on the eval set. Why is that number meaningless on its own?",
          answer: "Because you have not compared it to a baseline. Run the same eval on the base model with your best prompt; if the fine-tune does not clearly beat that prompt, the prompt wins because it is cheaper and more flexible.",
        },
        {
          prompt: "Beyond your target metric, what else must an honest evaluation check for?",
          answer: "Regressions and side effects. A fine-tune can get worse at adjacent tasks (catastrophic forgetting), so include general-capability checks, not just your narrow target, and re-run the eval whenever the data, epochs, or base model change.",
        },
      ],
    },
    {
...
- National Institute of Standards and Technology. (2023). *AI Risk Management Framework (AI RMF 1.0)*: see the "Govern" function (policies, legal, and accountability). https://www.nist.gov/itl/ai-risk-management-framework`,
      recallContent: [
        {
          prompt: "What are the two paths to run a fine-tuned model in production, and the main tradeoff of each?",
          answer: "Managed API fine-tuning, where there are no GPU ops and it scales for you but you pay to train, pay a higher per-token price, and accept vendor lock-in; and open-weights self-hosting, where you get full control, no per-token fee, and your data stays in your infra but you own the hosting and ops.",
        },
        {
          prompt: "Name one way fine-tuning can actually lower cost.",
          answer: "It can let you drop a giant system prompt, so you send fewer input tokens on every call. A smaller fine-tuned model can also beat a bigger general one on a narrow task, cheaper and faster per call.",
        },
      ],
    },
    {
...
- National Institute of Standards and Technology. (2023). *AI Risk Management Framework (AI RMF 1.0)*: Harmful Bias Managed; Privacy-Enhanced; "Measure"/"Manage." https://www.nist.gov/itl/ai-risk-management-framework`,
      recallContent: [
        {
          prompt: "The weights for an open model are downloadable. Can you automatically ship a commercial product on it?",
          answer: "No. 'Open weights' is not automatically 'open source' or 'do anything.' Licenses range from permissive (like Apache 2.0) to custom community licenses with real restrictions, so you must read that specific model and version's actual license, and flag it for legal review when unsure.",
        },
        {
          prompt: "What is the core difference between closed and open-weights models for fine-tuning?",
          answer: "With closed models like GPT or Claude you never hold the weights; you fine-tune through the provider's service and use the result through their API under their terms. With open-weights models you download the actual weights, fine-tune and host them yourself, gaining control and portability but owning the operations and license compliance.",
        },
      ],
    },
    {
...
**Step 3: Can you produce a real, honest dataset?** Can you write/curate enough **high-quality, consistent, representative** *(input, ideal output)* pairs (start ~50 to 100 and grow), correctly formatted, with a **held-out eval set**? If you can't build clean data, you can't fine-tune well. Fix that before training (Anthropic, 2024).
...
- National Institute of Standards and Technology. (2023). *AI Risk Management Framework (AI RMF 1.0)*: Map/Measure/Manage/Govern. https://www.nist.gov/itl/ai-risk-management-framework`,
      recallContent: [
        {
          prompt: "What are the three main risks of fine-tuning?",
          answer: "Catastrophic forgetting (the model gets worse at things it used to do), data leakage or privacy loss (it can reproduce training examples verbatim, leaking PII or secrets), and bias amplification (it learns and amplifies any bias in your data, then applies it at scale).",
        },
        {
          prompt: "What shared root cause do those three risks trace back to, and one defense that helps with all of them?",
          answer: "They trace back to the training data and over-training. Curating the data honestly and training no harder than you must (for example with PEFT/LoRA and fewer epochs), while evaluating for side effects, helps with all three.",
        },
      ],
    },
    {
        ...
        questions: [
          {
            ...
            options: [
              "It changes only the text you put into the model at inference time",
              "It updates the model's own weights, producing a customised version",
              "It retrains the entire model from scratch using your own data",
              "It makes the model deterministic for any repeated input text",
            ],
            correctIndex: 1,
            explanation:
          ...
          },
          {
            ...
            options: [
              "Full fine-tuning first, and then LoRA layered on top of it",
              "Nothing at all, because fine-tuning is always the first step",
              "A better prompt, and RAG if the gap is really knowledge",
              "Buy more GPUs so the training run finishes much sooner",
            ],
            correctIndex: 2,
            explanation:
          ...
          },
          {
            ...
            options: [
              "Updates every single weight throughout the pretrained model",
              "Freezes the pretrained weights and trains small low-rank matrices",
              "Retrieves matching documents from a vector store at query time",
              "Increases the maximum size of the model's own context window",
            ],
            correctIndex: 1,
            explanation:
          ...
          },
          {
            ...
            options: [
              "The largest possible row count that you can assemble",
              "Consistent, representative examples: quality over quantity",
              "Using only synthetic data generated by another model",
              "The fastest learning rate that your hardware allows",
            ],
            correctIndex: 1,
            explanation:
              "Quality beats quantity. Anthropic's guidance: start with ~50 to 100 high-quality rows, evaluate, then grow where the eval shows weakness.",
          ...
          },
          {
            ...
            options: [
              "88 percent is always too low a score to ship on",
              "Compare it to a baseline: your best prompt on the base model",
              "You need a bigger model before a score like that counts",
              "Eval sets are unreliable and should not gate a release",
            ],
            correctIndex: 1,
            explanation:
          ...
          },
          {
            ...
            options: [
              "When you want to do as little operations work as possible",
              "When data control, volume or cost-at-scale justify owning hosting",
              "Always, because open weights are strictly better in every case",
              "Only for models small enough to run on one single GPU card",
            ],
            correctIndex: 1,
            explanation:
          ...
          },
          {
            ...
            options: [
              "Using a context window that is far too small for the job",
              "The training data and over-training; curate and train no harder",
              "Running the model at a much too low sampling temperature",
              "Hosting the trained model with the wrong cloud provider",
            ],
            correctIndex: 1,
            explanation:
