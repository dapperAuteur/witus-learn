...
async function main() {
  ...
  // How to Run for Office — practical + strictly non-partisan (the *process*, not
  // who should win). Same Civics category and cited standard. Authoritative-values
  // rule is load-bearing: eligibility, ballot-access (signatures/fees), and
  // state/local contribution limits VARY by office/state, so the course names the
  // rule + points to the source (FEC, USA.gov, NCSL, Ballotpedia) rather than
  // asserting one number as universal.
  await seedAuthoredCourse(db, {
    tenantId: learnWitus,
    instructorId,
