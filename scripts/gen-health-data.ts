...
import { SOURCE_LESSON_OVERRIDES } from "./lib/source-lesson-overrides";
...
const matchTotals: MatchStats & { byHand: number } = { structural: 0, matched: 0, declined: 0, byHand: 0 };
...
/** Generated file slug -> the course slug the seeders register it under, which is how the hand
 *  overrides are keyed (a reader looking one up knows the course, not the filename). */
const COURSE_SLUG_OF: Record<string, string> = {
  "nasm-cpt-course": "nasm-cpt",
  "nasm-ces-course": "nasm-ces",
  "nasm-cnc-course": "nasm-cnc",
  "ecs-foundations-course": "ecs-foundations",
  "ecs-fitness-course": "ecs-fitness",
  "ecs-nutrition-course": "ecs-nutrition",
  "ecs-neuroscience-course": "ecs-neuroscience",
  "speedway-course": "speedway",
};

/**
 * Apply the hand assignments for questions the matcher refused to guess. Runs LAST, after every
 * automatic assignment, so a hand decision always wins.
 *
 * An override whose prompt matches nothing is REPORTED rather than ignored: that means the upstream
 * CentOS content changed under it, and a silent drop would put the question back to naming no lesson
 * with nobody noticing.
 */
function applyOverrides(fileSlug: string, course: AuthoredCourse): void {
  const map = SOURCE_LESSON_OVERRIDES[COURSE_SLUG_OF[fileSlug] ?? ""];
  if (!map) return;
  const unused = new Set(Object.keys(map));
  let applied = 0;
  for (const l of course.lessons) {
    for (const q of l.quiz?.questions ?? []) {
      const target = map[q.prompt];
      if (!target) continue;
      unused.delete(q.prompt);
      q.sourceLessonSlug = target;
      applied++;
    }
  }
  matchTotals.declined -= applied;
  matchTotals.byHand += applied;
  if (unused.size) {
    console.warn(
      `  ! ${unused.size} hand override(s) in source-lesson-overrides.ts matched no question in ${fileSlug}:`,
    );
    for (const p of unused) console.warn(`      ${p.slice(0, 90)}`);
  }
}

function emit(fileSlug: string, exportName: string, course: AuthoredCourse, note: string) {
  applyOverrides(fileSlug, course);
...
function main() {
  ...
  const { structural, matched, declined, byHand } = matchTotals;
  const assigned = structural + matched + byHand;
  const total = assigned + declined;
  console.log(
    `\nSource lessons: ${assigned}/${total} question(s) linked to the lesson that teaches the answer ` +
      `(${structural} structural, ${matched} matched, ${byHand} assigned by hand), ${declined} still declined.`,
  );
