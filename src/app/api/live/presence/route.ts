...
// separate check-in flow. Powers the parent Family view's attendance list. Presence AND
// attendance are attributed to the ACTIVE learner (self, or a managed child if a parent is
// "studying as" one) — cohort membership is checked for that same learner.
export async function POST() {
  const { sdb, session } = await apiContext();
  if (!session) return errorJson("Unauthorized", 401);
  const learner = (await getActiveLearner(session))!;
  await heartbeat(sdb.tenantId, learner.id, learner.name ?? null);

  const today = new Date().toISOString().slice(0, 10); // UTC YYYY-MM-DD
  const cohortIds = await listCohortIdsForMember(sdb.tenantId, learner.id);
  await Promise.all(cohortIds.map((cohortId) => recordAttendance(sdb.tenantId, cohortId, learner.id, today)));
