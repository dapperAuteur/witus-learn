...
// E2E + a11y gate (witus plan 30 Phase 2, ported from gemini/witus). Two ways to point it at a
// running app:
//   PLAYWRIGHT_BASE_URL=https://<deploy-url> pnpm exec playwright test   ← CI (Vercel preview/prod)
//   pnpm exec playwright test                                            ← local, expects dev server on :3040
// No webServer block on purpose: this repo's dev server needs real env (database, auth, email)
// that CI and fresh clones don't have. CI always runs against a deployed URL instead.
...
export default defineConfig({
  ...
  projects: [
    ...
    {
      // The ecosystem charter is mobile-first at 360px; a flow that only works on desktop is a
